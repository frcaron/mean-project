...
module.exports = {
	...
	create : function(req, res) {
		...
		TypeCategoryModel.findOne({ type : 'unknow' }, function(err, typeCategory) {
			if(err) return res.json(responseService.success('Add failed', err.message));
	...
	update : function(req, res) {
		...
		UserModel.findById(req.params.user_id, function(err, user) {
			...
			if(!user) return res.json(responseService.fail('Update failed', 'User not found'));
	...
	remove : function(req, res) {
		...
		// Query remove
		TransactionModel.remove({ _user : req.params.user_id}, function(err) {
			if(err) return res.json(responseService.fail('Remove failed', err.message));
		});	
		CategoryModel.remove({ _user : req.params.user_id}, function(err) {
			if(err) return res.json(responseService.fail('Remove failed', err.message));
		});			
		ProgramModel.remove({ _user : req.params.user_id}, function(err) {
			if(err) return res.json(responseService.fail('Remove failed', err.message));
		});		
		PlanModel.remove({ _user : req.params.user_id}, function(err) {
			if(err) return res.json(responseService.fail('Remove failed', err.message));
		});	

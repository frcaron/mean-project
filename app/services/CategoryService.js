...
module.exports = {
	...
	remove : function(req, res) {
		...
		// Query find category by id and user
		CategoryModel.findOneAndUpdate({ _id : req.params.category_id, _user : req.decoded.id}, function(err, category) {
			if(err) {
	...
	allActiveByU : function(req, res) {
		...
		// Query find categories by user
		CategoryModel.find({ _user : req.decoded.id, active : true }, function(err, categories) {
				if(err) {
					return res.json(responseService.fail('Find failed', err.message));
				}
				return res.json(responseService.success('Find success', categories));
	...
	allByTypeCategoryU : function(req, res) {
		...
		CategoryModel.find({ _user : req.decoded.id, type : req.params.type_category_id, active : true }, function(err, categories) {
				if(err) {
					return res.json(responseService.fail('Find failed', err.message));
				}
				return res.json(responseService.success('Find success', categories));
